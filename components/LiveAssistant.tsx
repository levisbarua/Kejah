...
export const LiveAssistant: React.FC = () => {
  ...
  const startSession = async () => {
    ...
    try {
      ...
      // Attempt to create contexts with desired sample rate, but fallback if necessary
      // Note: Browsers will automatically resample if hardware doesn't support specific rate
      ...
      // CRITICAL: Resume contexts to ensure they are not suspended
      ...
      // We create a new instance to ensure we pick up the latest API key if it changed
      ...
      const callbacks = {
        onopen: async () => {
          ...
          // Start Microphone Stream only after connection opens
          try {
            ...
            // Buffer size 4096 gives good balance of latency/stability (approx 256ms at 16k)
            ...
            processor.onaudioprocess = (e) => {
              ...
              // Use the standardized PCM conversion
        ...
        onerror: (err: any) => {
          ...
          // Don't immediately stop session here to allow user to see error, 
          // or rely on user to close. But often best to reset state.
          setIsConnecting(false);
        }
      };
